...
export type MenuItem = {
  ...
  icon?: ReactNode;
  disabled?: boolean;
  defaultStyles?: { bgColor?: string; textColor?: string };
  hoverStyles?: {
    bgColor?: string;
    textColor?: string;
  };
...
export default function BubbleMenu({
  ...
  const [showOverlay, setShowOverlay] = useState(defaultOpen);
        ...
        >
          ...
          >
            {menuItems.map((item, idx) => {
              const pillBg = item.defaultStyles?.bgColor ?? menuBg;
              const pillColor = item.defaultStyles?.textColor ?? menuContentColor;
              const pillClassName = [
                'pill-link',
                'w-full',
                'rounded-[999px]',
                ...
                item.disabled ? 'pointer-events-none opacity-70 cursor-not-allowed' : ''
              ].join(' ');
              const pillStyle = {
                '--item-rot': `${item.rotation ?? 0}deg`,
                '--pill-bg': pillBg,
                '--pill-color': pillColor,
                '--hover-bg': item.hoverStyles?.bgColor || pillBg,
              ...
              const content = (
                <>
                  {item.icon && (
                    <span className="pill-icon flex-shrink-0 inline-flex items-center justify-center" style={{ color: 'inherit', filter: item.disabled ? 'grayscale(1)' : undefined }}>
                      {item.icon}
                    </span>
                  )}
                  <span
                  ...
                  {item.disabled ? (
                    ...
                    <a
                      ...
                      onClick={onItemClick ? (e) => {
                        ...
                        if (showToggle) setIsMenuOpen(false);
                      } : undefined}
                    >
                      {content}
                    </a>
